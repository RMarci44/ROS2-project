...
def generate_launch_description():
    ...
    pkg_bme_ros2_project = get_package_share_directory('bme_ros2_project')

    gazebo_models_path, ignore_last_dir = os.path.split(pkg_bme_ros2_project)
    ...
    urdf_file_path = PathJoinSubstitution([
        pkg_bme_ros2_project,  # Replace with your package name
        "urdf",
        LaunchConfiguration('model')  # Replace with your URDF or Xacro file
    ])

    gz_bridge_params_path = os.path.join(
        get_package_share_directory('bme_ros2_project'),
        'config',
        'gz_bridge.yaml'
    )
    ...
    world_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_bme_ros2_project, 'launch', 'world.launch.py'),
        ),
        launch_arguments={
    ...
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', PathJoinSubstitution([pkg_bme_ros2_project, 'rviz', LaunchConfiguration('rviz_config')])],
    ...
    ekf_node = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(pkg_bme_ros2_project, 'config', 'ekf.yaml'),
            {'use_sim_time': LaunchConfiguration('use_sim_time')},
             ]
    )
